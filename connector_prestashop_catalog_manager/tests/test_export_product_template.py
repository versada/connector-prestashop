...
class TestExportProduct(CatalogManagerTransactionCase):
    ...
    def test_export_product_template_wizard_export(self):
        # export from wizard
        wizard = self.env['export.multiple.products'].with_context(
            active_ids=[self.template.id]).create({})
        wizard.export_products()

        # check binding created
        binding_model = 'prestashop.product.template'
        bindings = self.env[binding_model].search(
    ...
    def test_export_product_template_wizard_active(self):
        ...
        wizard = self.env['active.deactive.products'].with_context(
            active_ids=[self.template.id]).create({})
        ...
        # check export delayed
        self.assertEqual(
            3, self.instance_delay_record.export_record.call_count)
        # activate again
        wizard.active_products()
        # check no export delayed
        self.assertEqual(
        ...
        # check export delayed
        self.assertEqual(
            4, self.instance_delay_record.export_record.call_count)
    ...
    def test_export_product_template_wizard_resync(self):
        ...
        self._bind_template()
        # resync from wizard
        wizard = self.env['sync.products'].with_context(
            active_ids=[self.template.id], connector_delay=True).create({})
    ...
    def test_export_product_template_job(self):
        ...
        with recorder.use_cassette(
            ...
            for field, value in {
                    ...
                    'description_short': '<p>New product description short'
                                         '</p>',
                ...
                self.assertEqual(
                    value, ps_product[field]['language']['value'])
