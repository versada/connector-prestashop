...
class TestImportInventory(PrestashopTransactionCase):
    ...
    def test_01_import_inventory_delay(self):
    ...
    @assert_no_job_delayed
    def test_02_import_inventory_batch(self):
        delay_record_path = "odoo.addons.queue_job.models.base.DelayableRecordset"
        # execute the batch job directly and replace the record import
        # by a mock (individual import is tested elsewhere)
        with recorder.use_cassette(
            "test_import_inventory_batch"
        ) as cassette, mock.patch(delay_record_path) as delay_record_mock:
            ...
            # imported
            delay_record_instance = delay_record_mock.return_value
            self.assertEqual(45, delay_record_instance.import_record.call_count)

    @assert_no_job_delayed
    def test_03_import_inventory_record_template(self):
        ...
        with recorder.use_cassette("test_import_inventory_record_template_1"):
            self.env["_import_stock_available"].import_record(
                ...
                record={"id_product_attribute": "0", "id": "1", "id_product": "1"},
            )
    ...
    @assert_no_job_delayed
    def test_04_import_inventory_record_variant(self):
        ...
        with recorder.use_cassette("test_import_inventory_record_variant_1"):
            self.env["_import_stock_available"].import_record(
                self.backend_record,
                1,
                record={"id_product_attribute": "1", "id": "1", "id_product": "1"},
            )
        template = self.env["product.template"].browse(template.id)
